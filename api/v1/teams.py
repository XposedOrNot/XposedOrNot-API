...
async def setup_teams_channel_endpoint(
    ...
    """Set up, verify or delete the Teams channel of a verified domain."""
    try:
        ...
        if channel_data.action == "delete":
            success = await delete_teams_channel(channel_data, email)
            if not success:
                raise HTTPException(
...
async def get_teams_channel_config_endpoint(
    ...
    """Get the Teams channel configuration for a verified domain."""
    try:
        if not validate_url(request):
            raise HTTPException(status_code=400, detail="Invalid request URL")
        ...
        config = await get_teams_channel_config(domain, owner)
        if not config:
